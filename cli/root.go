// Package cli builds the mw command tree on top of the mathworld library.
package cli
...
type App struct {
	client *mathworld.Client
	cfg    mathworld.Config
...
func Root() *cobra.Command {
	app := &App{cfg: mathworld.DefaultConfig()}

	root := &cobra.Command{
		Use:   "mw",
		Short: "Browse Wolfram MathWorld mathematical articles",
		Long: `mw reads Wolfram MathWorld, the web's most extensive mathematical resource.
It returns articles as table, JSON, JSONL, CSV, TSV, or URLs.

mw is an independent tool and is not affiliated with Wolfram Research.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = mathworld.NewClient(a.cfg)
	return nil
}
