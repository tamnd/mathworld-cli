...
describes what users get. Pick a format with `-o`, or let mw choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mw <command> -o table   # aligned columns for reading
mw <command> -o jsonl   # one JSON object per line, for piping
mw <command> -o json    # a single JSON array
mw <command> -o csv     # spreadsheet friendly
mw <command> -o tsv     # tab-separated
mw <command> -o url     # just the URL column
mw <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mw <command> --fields id,title,url
```
...
```bash
mw <command>            # a table, because this is a terminal
mw <command> | wc -l    # JSONL, because this is a pipe
```
